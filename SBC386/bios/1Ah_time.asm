...
int_1Ah:
	...
	add	si,si		; address words

	cmp	si,len_dispatch0  ; length is in words
	jb	low_dispatch2

	sub	si,0x20 * 2	; calls based at 20h
	cmp	si,len_dispatch20  ; length is in words
	jb	exec20_dispatch
...
low_dispatch2:
   cs	call	[dispatch0 + si]
	jmp	exit

exec20_dispatch:
   cs	call	[dispatch20 + si]
	jmp	exit
...
dispatch20:
	...
	dw	ds_write	; 21h
	dw	wr_protect	; 22h
	dw	ds_set_charge	; 23h
	dw	ds_get_charge	; 24h
...
; diagnostic aid
;	AL = read command
; 	AX has byte read in AL
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	global	ds_read_reg_
ds_read_reg_:
	pushm	dx
	mov	ah,al			; move command to position
	or	ah,1000_0001B
	call	ds_reset_on
	call	ds_reset_off
	call	ds_outb
...
.2:
; handle the FDC motor timeout
	cmp	[motor_count],ch	; CH==0
	je	.3
...
; restore before clearing the interrupt
	popm	bx,cx,ds
...
; Timer 0 constants
;
divide	equ	MS18 		; 50619
tick	equ	CLKIN0*86400/divide
ttime	equ	(divide*1000000)/(CLKIN0)


t0tab:
	binit	TMRCON, CTR0 + CWR01 + MODE2 + ZBIT0	;binary
	binit	TMR0, MS18LO	;low byte
	binit	TMR0, MS18HI	;high byte = 46080
	binit	TMRCFG, TIMER_STOP + BIT1		; start timer 0
        dw      -1
